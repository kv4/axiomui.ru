...
> **Это репозиторий сайта и ботов.** Бизнес-материалы практики живут отдельно, в приватном
> репозитории `~/work/axiomui-business`: `CONTEXT.md` (источник истины — стратегия, воронка, NDA),
> `PLAN.md` (задачи), `growth/` (лиды, письма, методики). Перед работой над текстами, смыслами
> и стратегией читай их оттуда; сюда бизнес-материалы добавлять не нужно.
>
> Агент писем `outreach-writer` и команды `/letter`, `/send-day` живут там же — в сессии,
> открытой в этом каталоге, они не подхватятся. Для писем открывай сессию в бизнес-репозитории.
